...
| 主要数据存储 | 用户选择的本地文件；系统临时目录中的单份草稿和主题配置 |
...
| 图片输入 | Markdown 中的绝对或相对本地路径 | 只通过本地资源协议读取 PNG、JPEG、GIF、WebP、BMP、SVG |
| 文档输出 | Markdown、主题化 HTML、主题化 PDF、剪贴板 | HTML/PDF 接收活动主题和字号；输出失败时保留编辑区内容和当前预览 |
| 临时数据 | 单份草稿、已导入主题 | 写入系统临时目录，可能被操作系统清理 |
...
`MdEditorApp.tabs` 保存全部标签，`active_tab` 指向当前标签。当前标签同时映射到应用工作字段，切换前写回原标签，切换后载入目标标签。
...
5. 切换标签前写回活动状态，切换后编辑、状态栏、目录、复制、导出和预览都读取新标签；
6. 重复打开同一路径时切换到已有标签，不创建第二份内存副本；
7. 关闭有修改或冲突的标签前必须取得保存、放弃或取消选择；
8. 关闭最后一个标签后立即创建空标签；
9. 主题导入失败时保留原主题和字号；
10. 恢复草稿只修改内存文本，用户保存前不改变原文件；
...
阅读模式从内部块模型提取 H1–H6，保留标题层级。点击目录后，WebView 按章节索引平滑滚动到对应标题。
...
草稿保存条件为：当前文档非 `Saved`、文本非空、停止输入超过 30 秒、距离上次草稿写入超过 30 秒。启动时发现非空草稿会询问恢复或放弃。
...
| 进程异常结束 | 下次启动可能提示单份草稿 | 最近活动文档 | 恢复草稿后手动保存 | 草稿缺少源路径和时间 |
...
| 多文档 | 单窗口多标签，共享 WebView | 切换成本低，减少浏览器实例数量 | 活动标签有工作字段镜像；异常恢复复杂 | 需要多窗口、标签拖拽或跨窗口移动 |
...
| 临时持久化 | 系统临时目录单文件 | 实现简单，无配置迁移 | 可能被清理，无法可靠恢复多标签 | 需要可靠恢复和跨版本配置 |
...
flowchart LR
  ...
  Host <--> |草稿、主题 JSON| Temp[("系统临时目录")]
  Host -->|ProgID / Document Types| OS["操作系统应用关联服务"]
```

桌面宿主进程是主要应用容器。系统 WebView 是由平台浏览器运行时承载的子视图。文档目录和临时目录是两个用途不同的数据存储：前者属于用户内容，后者只保存可丢失的会话辅助数据。
...
3. 用文件对话框、拖放和双击关联文件打开文档；
4. 让外部进程连续写入已打开文件，分别验证自动重载、内容相同和本地冲突；
5. 在长文档中验证目录定位和两侧双向平滑同步；
6. 打开菜单和关闭确认窗口，检查 WebView 不遮挡弹层；
7. 在 Intel 与 Apple Silicon macOS 上验证 App 启动、Finder 打开方式和文档打开事件；
8. 检查 GitHub Release 只包含 Windows Setup、Windows ZIP 和 macOS Universal App ZIP。
9. 在固定 Windows 性能机执行 ADR-0003 三档基准，保存 `latest.json` 并检查所有预算。

### 8.2 已知缺口

| 优先级 | 缺口 | 完成条件 |
| --- | --- | --- |
| P0 | 单份草稿无法恢复多个未保存标签 | 草稿保存标签 ID、路径、文本、时间和版本；重启后逐项恢复 |
| P0 | 关闭整个窗口时没有逐标签保存确认 | 捕获关闭请求，列出全部未保存标签，支持保存、放弃和取消退出 |
| P1 | 外部轮询可能漏掉相同时间戳、相同长度的改写 | 引入可靠文件通知或内容摘要，并增加模拟测试 |
| P1 | 活动标签同时存在工作字段和 `DocumentTab` 镜像 | 让界面直接操作活动标签，移除手动写回路径 |
| P1 | 草稿和主题使用临时目录 | 迁移到用户配置目录，增加版本、损坏降级和清理规则 |
| P1 | macOS Finder 文档事件缺少自动化验证 | 验证冷启动和应用已运行两种情况；必要时接入 `application:openURLs:` |
| P2 | 文件关联每次启动新进程，不能复用现有窗口 | 加入单实例通信，把新路径发送到现有进程并创建标签 |
