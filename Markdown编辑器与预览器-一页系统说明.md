...
- 写作、阅读、分栏和专注模式。
- CommonMark 基础语法，以及表格、脚注、删除线、任务列表和围栏代码块。
- Windows 上使用 WebView2 生成实时预览；JavaScript 关闭。
...
| 运行依赖 | Windows 10/11、WebView2 Runtime | WebView2 不可用时浏览器预览无法创建 |
...
| Q7 | 应用离线启动 | 内置字体和内置主题可用 | 字体与 CSS 已嵌入可执行文件；外部主题中的远程资源不受此承诺约束 |
...
8. JavaScript 在 WebView2 中保持关闭；若未来开启，必须单独评估主题包和文档内容的安全边界。
...
4. HTML 哈希变化时调用 WebView2 `load_html`；哈希相同时结束。
5. 编辑区拥有焦点时，把焦点从子 WebView2 交还父窗口，保证左侧可继续输入。
...
顶部菜单、标签标题和视图切换统一使用 13 px 字号；交互区域统一为 30 px 高，并在 36 px 顶部栏内垂直居中。当前标签和当前视图的底线使用同一纵向基准。
...
- 安全边界：JavaScript 关闭；CSS 中的 `url()` 仍可能请求本地或远程资源。
...
| `web_preview.rs` | HTML 生成、CSS 层叠、字体协议、WebView2 生命周期 | `document`、`BrowserPreview::show/hide` | 浏览器 DOM 兼容和预览更新 |
