...
`FileStamp` 用修改时间和长度判断文件是否可能变化。应用同时为每个已绑定路径注册操作系统文件变更通知；通知到达时立即读取完整字节，即使文件戳和长度没有变化，也能识别 Agent 的原位改写。通知不可用或丢失时，保留每 350 ms 的轮询兜底。候选变化先读取完整字节，再等待至少 450 ms 且文件戳保持稳定，稳定后才进入文本替换或冲突判断。
...
4. 深色模式覆盖层（仅深色外观），使用当前主题的深色色板并设置 `color-scheme: dark`。

深色模式覆盖应用画布、编辑区、预览 `html/body`、标题、链接、引用、代码块、表格交替行和分隔线。原始主题 CSS 仍然先执行，覆盖层只处理这些语义表面，不替换主题的布局、字体和组件结构。
...
操作系统通知用于降低外部写入的发现延迟，轮询用于跨平台兜底。稳定等待避免读取 Agent 正在分段写入的文件；重复通知不会重置首次发现时间，只有内容或文件戳实际变化时才会更新候选。通知与轮询均读取完整字节并复用同一冲突状态机。
...
“深色/浅色外观”偏好单独保存到用户配置目录的 `appearance.json`，带格式版本、时间戳、原子替换、损坏隔离和 7 天清理规则。启动时先读取偏好，再选择主题的亮色或深色色板；缺失、损坏或版本不兼容时默认为亮色。切换外观会使浏览器文档缓存失效，预览和导出在同一帧使用新的色板。
