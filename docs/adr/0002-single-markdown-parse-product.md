...
> 日期：2026-08-13
> 影响版本：下一发布版本起
> 更新：2026-09-16 浏览器预览通道已移除，见文末“后续变更”
...
- 浏览器预览通道已整体移除：`web_preview.rs` 删除，消费者规则第 2 条不再适用。预览由 `preview.rs` 的 egui 混合编辑界面直接消费 `ParsedDocument.blocks`，解析与渲染的异步衔接见 ADR-0004（Core + Worker + Snapshot）。
- `export.rs` 仍从同一 `events` 流生成导出 HTML/PDF（第 3 条消费者继续成立）；文中其余 `web_preview.rs` 相关表述仅具历史意义。
- 经审计，"Parser::new 只能出现在 markdown.rs" 的约束仍然成立（`rg -n "Parser::new" src` 仅命中 `markdown.rs`）。
