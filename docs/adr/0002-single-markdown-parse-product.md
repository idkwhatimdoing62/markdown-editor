...
两条路径共用了部分解析选项和兼容预处理，但每个消费者仍能独立改变解析入口。新增语法可能只进入一侧，造成目录、后备预览、浏览器预览和导出对同一份源码理解不同。重复解析还会增加长文档编辑时的计算和临时分配。
...
- 增量路径只在兼容预处理未产生规范化文本、且新旧事件结构可映射时启用，语义不确定时优先保证正确性；
