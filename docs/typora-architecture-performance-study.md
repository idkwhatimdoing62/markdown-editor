...
这是最接近 Typora“文档表面连续更新”体验的方案，也是预计收益最大的改动。`RenderWorker` 会为接受的结果记录渲染阶段耗时、替换块数、虚拟分片数和完整渲染回退次数；`BrowserPreview` 单独记录已有 WebView 的真实导航次数，并保留最近样本计算 P95。验收指标还应包括滚动锚点漂移。
