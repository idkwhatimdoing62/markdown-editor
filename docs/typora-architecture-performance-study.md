...
项目已经有缓存、200 ms 防抖、超过 512 KiB 或 8 张图片时的虚拟块、Mermaid 延迟加载、WOFF 字体和隐藏时释放 WebView。当前解析层已为段落、标题、代码块和分隔线实现保守的单块增量更新：未受影响的 `Block` 与 `SpannedEvent` 会复用，后续事件范围按字节差量平移；列表、引用、表格、脚注和 HTML 等可能跨块传播语义的编辑仍回退完整解析。这些优化解决的是首屏和容量问题；与 Typora 的主要差距在于浏览器 HTML 仍需由后台任务生成后再做块补丁。
