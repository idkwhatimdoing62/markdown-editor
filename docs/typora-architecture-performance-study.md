...
项目已经有缓存、200 ms 防抖、超过 512 KiB 或 8 张图片时的虚拟块、Mermaid 延迟加载、WOFF 字体和隐藏时释放 WebView。这些优化解决的是首屏和容量问题；与 Typora 的主要差距在于编辑时的更新粒度仍偏粗：缓存命中依赖完整文档版本，文本编辑可能使当前文档重新解析并重建较大范围 HTML。
