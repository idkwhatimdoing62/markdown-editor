...
- **对比度**：正文、次要文字（muted）、标题、强调色、代码块语言标签与代码注释都必须达到 4.5:1（WCAG AA），专注模式下被弱化的文字同样适用——代码注释色由正文色与代码底色派生，弱化幅度以这条基线为界（`code_comment_color`）。内置主题的浅色与深色两套配色由单元测试逐项校验，改坏取色会直接让测试失败。
